...
fn wait_attach_until_close<F, R>(mut f: F)
where
    F: FnMut(&Process, &mono::Module, &mono::Image, &HKConfig) -> R,
{
    ...
    loop {
        ...
        log::info!("GameManagerFinder wait_attach: waiting for attach...");
...
pub fn text_masher(
    mut do_key_event: impl FnMut(u8),
